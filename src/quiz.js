...
function Quiz() {
    ...
    fetch(getQuestionsUrl
      ...
      .then((data) => {
        const result = data.results.map((item) => {
          const { correct_answer: correctAnswer, incorrect_answers: incorrectAnswers, ...rest } = item;
          return ({
          ...rest,
          correctAnswer,
          incorrectAnswers,
  ...
  return (
    <div className="App">
      ...
      {Object.keys(selectedAnswers).length === questions.length &&  Object.keys(selectedAnswers).length > 0 && (<button onClick={() => navigate("/result", { state: { selectedAnswers, questions } })} style={styles.footerButton}>Submit</button>)}
      </section>
    </div>
  );
}
